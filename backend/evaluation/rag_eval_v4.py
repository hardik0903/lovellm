...
class GeminiClient:
    """
    Async wrapper around google-generativeai for Gemini 3.1 Flash Lite.

    Enforces the free-tier rate limits (RPM=15, TPM=250K, RPD=500) via
    an internal RateLimiter so every caller is automatically throttled.
    ...
    def __init__(self):
        ...
        try:
            ...
            print(
                ...
                f"TPM={RATE_LIMIT_TPM:,}  RPD={RATE_LIMIT_RPD}"
            )
    ...
    async def generate(
        ...
        """
        Rate-limited call to Gemini 3.1 Flash Lite.
        Blocks at acquire() if any RPM / TPM / RPD limit is near exhaustion,
        then runs the synchronous SDK call in a thread executor.
        """
        ...
        # Block until within all rate limits
        ...
        try:
            ...
            # Update token reservation with actual usage if available
            try:
            ...
            err = str(exc)
            # Surface quota errors clearly; they should be rare with the limiter in place
            if "429" in err or "quota" in err.lower() or "rate" in err.lower():
                print(f"  [GEMINI] Rate-limit 429 despite throttling — backing off 60s: {err[:120]}")
                await asyncio.sleep(60)
            else:
                print(f"  [GEMINI] generate() error: {err[:120]}")
            return ""
...
async def main(
    ...
    print("\n[Step 2] Initialising Gemini 3.1 Flash Lite client...")
    gemini_client = GeminiClient()
    if not gemini_client.enabled:
        print(f"  FATAL: {gemini_client._init_error}")
        print("  Set GEMINI_API_KEY and ensure google-generativeai is installed.")
        return {}
    ...
    for doc_idx, doc in enumerate(documents):
        ...
        try:
            ...
            # approaching RPD exhaustion early (500 req/day burns fast at
            # 3 calls/question × N questions per doc).
            rl_status = gemini_client.rate_limiter.status()
            print(
                f"  [RATE] After doc {doc_idx + 1}: "
                f"RPM {rl_status['rpm_used']}/{rl_status['rpm_limit']}  "
                f"TPM {rl_status['tpm_used']:,}/{rl_status['tpm_limit']:,}  "
                f"RPD {rl_status['rpd_used']}/{rl_status['rpd_limit']}"
            )
            remaining_rpd = rl_status["rpd_limit"] - rl_status["rpd_used"]
            if remaining_rpd <= 50:
                print(
                    f"  [RATE] ⚠  Only {remaining_rpd} daily requests remaining. "
                    f"Consider re-running tomorrow or using --subset."
                )
    ...
    if output_path:
        output = {
            "metadata": {
                ...
                "rate_limit_usage":       gemini_client.rate_limiter.status(),
